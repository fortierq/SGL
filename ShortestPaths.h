...
namespace sgl
{
	...
	{
		const Graph& G;
		typedef std::tuple<int, Edge*> elem; // Vertex, edge giving the predecessor
	public:
		bool sameEdges;
		/*! Minimum spanning tree giving predecessors, from leafs to root
		\warning There is at most one edge for each vertex (giving its predecessor), you can't access the child of a vertex
		...
		*/
		bool operator()(int source = 0)
		{
			...
			for (Edge* e = it.beg(); !it.end(); e = it.nxt())
				S.push(std::make_tuple(e->other(source), e));
			while (!S.empty())
			{
				...
				if (sameEdges)
					MST.insert(std::get<1>(S.top()), v);
				else
					MST.insert(new Edge(*(std::get<1>(S.top()))), v);
				S.pop();
				if (++nEdges == G.V() - 1)
					return true;
	...
	{
		const Graph& G;
		typedef std::tuple<type_wt, int, Edge*> elem; // distance, vertex, predecessor
	public:
		/*! \details dist[v]:
		...
		{
			std::priority_queue<elem, std::vector<elem>, std::greater<elem> > S;
			S.push(std::make_tuple((type_wt)0, source, nullptr));
			while (!S.empty())
			{
				type_wt d_v;
				int v;
				Edge* pere;
				std::tie(d_v, v, pere) = S.top();
				S.pop();
				...
				for (Edge* e = it.beg(); !it.end(); e = it.nxt())
				{
					...
					if (dist[w] > cost)
						S.push(std::make_tuple(cost, w, e));
				}
			}
		}
	};
	...
	public:
		...
		bool operator()(int source = 0)
		{
			...
			for (int N = 0; N < G.V(); N++) // G.V() + 1 relaxations, + 1 ??
			{
				...
				for (size_t i = 0; i < addToQ.size(); i++)
